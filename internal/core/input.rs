...
/// From backend code, they can be created from a list of string parts with the same
/// syntax as the macro:
///
/// ```rust
...
enum ShiftBehavior {
    ...
    /// Bindings involving this key automatically get `ignore_shift = true`.
    LocalizedShiftable,
...
fn lookup_key_name(name: &str) -> Option<(char, ShiftBehavior)> {
    macro_rules! check_key_name {
        ($($char:literal # $key_name:ident # $($shifted:expr)? $(=> $($qt:ident)|* # $($winit:ident $(($_pos:ident))?)|* # $($xkb:ident)|*)? ;)*) => {
            $(
    ...
    macro_rules! shift_behavior_for {
        ...
        ($shifted:expr) => {
            ShiftBehavior::LocalizedShiftable
        };
    }
    i_slint_common::for_each_keys!(check_key_name);
...
fn keys_from_parts<'a>(parts: impl Iterator<Item = &'a str>) -> Result<Keys, KeysParseError> {
    ...
    if let Some((key_char, shift_behavior)) = lookup_key_name(key_name) {
        ...
        let key = normalize_key(&key_char.to_lowercase().collect::<alloc::string::String>());
        return Ok(Keys { key, modifiers, ignore_shift, ignore_alt });
    }
    ...
    if grapheme_count == 0 {
        return Err(KeysParseError::UnknownKey(key_name.into()));
    }
    if grapheme_count > 1 {
